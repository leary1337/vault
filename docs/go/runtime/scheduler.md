...
**Планировщик Go** — это компонент, который распределяет задачи между потоками. В контексте языка Go планировщик управляет выполнением [Goroutine](../concurrency/goroutines.md) — легковесных потоков, созданных на уровне приложения. Планировщик Go разработан для эффективного использования ресурсов, таких как процессорное время и память, что особенно важно при разработке веб-сервисов с большой нагрузкой ввода-вывода (I/O-bound).
...
- **G ([Goroutine](../concurrency/goroutines.md))** — легковесный поток, который исполняет код (что исполняем).
