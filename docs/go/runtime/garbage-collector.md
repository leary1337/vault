...
Так продолжается до тех пор, пока не закончатся серые объекты. В этой фазе GC *забирает до 25% от CPU*, это число зашито внутри [Runtime Go](runtime-overview.md).
...
2. Прошло 2 минуты без GC. Да, даже если вы ничего не аллоцировали за это время, GC все равно запустится раз в 2 минуты. За это ответственен [Sysmon](runtime-overview.md). Этот способ можно отключить, выставив значение GOGC < 0. 
