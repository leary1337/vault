...
**Замыкания (closures) в Go** — это [функции](functions.md), которые "замыкаются" на переменные из внешнего контекста, в котором они были созданы. Замыкания могут сохранять и изменять значения этих переменных, даже после завершения внешней функции.
