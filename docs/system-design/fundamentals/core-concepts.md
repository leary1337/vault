...
**L4:** Балансировка на [сетевом и транспортном уровнях](../../networking/fundamentals/osi.md). Работает с IP-адресами и портами, не анализируя содержимое запросов. Используется для балансировки TCP и UDP трафика.
...
**L7 (уровень 7):** Балансировка на [прикладном уровне](../../networking/fundamentals/osi.md). Анализирует HTTP-запросы и другие протоколы прикладного уровня для принятия решений о распределении нагрузки.
...
DNS балансировка распределяет трафик на уровне [DNS](../../networking/application/dns.md), предоставляя разные IP-адреса для одного доменного имени. Когда пользователь запрашивает доменное имя, DNS-сервер возвращает один из множества IP-адресов, распределяя таким образом нагрузку между несколькими серверами.
